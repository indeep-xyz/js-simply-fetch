...
SimplyFetch.prototype.fetch = function(urlSources) {
  ...
  function loadFile(urlSource){
    ...
    var loadEventListener = xhr.addEventListener('load', afterLoadedfunction(event){
    ...
    var errorEventListener = xhr.addEventListener('error', function(event){
      xhr.removeEventListener('error', errorEventListener);
      raiseError();
    });
